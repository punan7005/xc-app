...
public class UUAPIcom {
	...
	public String userLogin(String userName, String passWord) {
		...
		this.userAgent = Md5.MD5(this.softKEY.toUpperCase()
		...
		if (!result.isEmpty()) {
			...
			this.uid = uids[0];
			this.softContentKEY = Md5.MD5((userKey + softID + softKEY)
					.toLowerCase());
			this.gkey = Md5.MD5((softKEY + userName).toUpperCase())
					+ macAddress;
				...
				if (!postData.isEmpty() && postData.size() > 0) {
					...
					for (Entry<String, String> ent : ((Map<String, String>) postData)
						...
						if (ent.getKey().equals("img"))
							...
							parts[temp] = new StringPart(ent.getKey(),
									ent.getValue());
						temp++;
					}
	...
	public String regUser(String userName, String userPassword) {
		if (softID.isEmpty() && softKEY.isEmpty()) {
			if (userName.isEmpty() && userPassword.isEmpty()) {
				...
				data.put(
						"UKEY",
						Md5.MD5(userName.toUpperCase() + userPassword + softID
								+ softKEY.toLowerCase()));
	...
	public String pay(String userName, String Card) {
		if (softID.isEmpty() && softKEY.isEmpty()) {
			if (userName.isEmpty() && Card.isEmpty()) {
				...
				data.put(
						"pkey",
						Md5.MD5(userName.toUpperCase() + softID + softKEY
								+ Card.toUpperCase()));
	...
	public static void main(String[] args) {
		String result = null;
		String imgCode = null;
		String imgCodeId = null;
		long curr = System.currentTimeMillis();
		UUAPIcom ucom = new UUAPIcom();
		ucom.setSoftInfo("107380", "1142ec34f7124149b29cfe0c2c889543");
		ucom.userLogin("punan7005", "dama!321");
		result = ucom.upload("C:\\jokerpu\\temp\\14442949109680.14544345972233552.jpg", "1004", true);
		System.out.println(result);
		while (!StringUtils.isNotBlank(imgCode)) {
			String[] arrayResult = result.split("\\|");
			if (arrayResult.length > 1) {
				imgCode = arrayResult[1];
			}else{
				if(!StringUtils.isNotBlank(imgCodeId)){
					imgCodeId = arrayResult[0];
				}else{
					result = ucom.getResult(imgCodeId);
					if(result.length() == 4){
						imgCode = result;
					}
				}
